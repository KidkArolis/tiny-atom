...
test.serial('usage', async function (t) {
  ...
  const { root: rootEl, assert, render, unmount } = testApp({ h, Provider, Consumer, connect })
  let root
  render((App) => {
...
test.serial('usage with createContext, createConsumer and createConnect', async function (t) {
  const h = (global.h = React.createElement)
  const { root: rootEl, assert, render, unmount } = testApp({ h, createContext, createConnect, createConsumer })
  let root
  render((App) => {
    root = createRoot(rootEl)
    root.render(h(App))
